...
function centralNormalizeRecord(row){
  ...
  Object.keys(row).forEach(function(k){
    ...
    if(o[camel]===undefined || o[camel]===null || o[camel]==='') o[camel]=row[k];
  });
  /* Alias usados por las planillas */
  if(o.nombreColaborador===undefined) o.nombreColaborador=o.nombre||'';
  if(o.tipoAusencia===undefined) o.tipoAusencia=o.tipo||'';
  if(o.numeroDias===undefined) o.numeroDias=o.dias;
  if(o.fechaInicio===undefined) o.fechaInicio=o.inicio;
  if(o.fechaTermino===undefined) o.fechaTermino=o.termino;
  if(o.sucursal===undefined) o.sucursal=o.sucursalActual||'';
  return o;
}
function centralNormalizeArray(arr){ return (arr||[]).map(centralNormalizeRecord); }
...
function centralLoadState(){
  ...
  return centralRequest('/state').then(function(result){
    var s = result.state || {};
    state.colaboradores = centralNormalizeArray(s.colaboradores);
    state.sucursales = centralNormalizeArray(s.sucursales);
    state.traslados = centralNormalizeArray(s.traslados);
    state.licencias = centralNormalizeArray(s.licencias);
    state.vacaciones = centralNormalizeArray(s.vacaciones);
    state.usuarios = centralNormalizeArray(s.usuarios);
    state.auditoria = centralNormalizeArray(s.auditoria);
...
dbGetAll = function(storeName){
  ...
  return centralRequest('/state').then(function(result){
    ...
    return centralNormalizeArray(rows);
  });
};
