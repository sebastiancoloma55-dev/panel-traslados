...
index_path = Path(file).with_name("index.html")
if not index_path.exists():
st.error("No se encontró index.html junto a app.py.")
st.stop()

html_content = index_path.read_text(encoding="utf-8")
html_content = html_content.replace("</head>", '\n<style id="iframe-fill-final">\nhtml,body{\n  background:#eef8f2 !important;\n  min-height:100vh !important;\n}\n.app-shell{\n  min-height:100vh !important;\n  background:#eef8f2 !important;\n}\n.main{\n  min-height(100vh - 18px) !important;\n  background:#eef8f2 !important;\n}\n.main > .tab-panel{\n  min-height(100vh - 110px) !important;\n}\n</style>\n', 1)
...
var data = {};
try { data = text ? JSON.parse(text) : {}; } catch(e) { data = { error || 'Respuesta inválida' }; }
if(!r.ok){
...
return centralRequest('', { method:'POST', body:{ action:'upsert', table(storeName), record } }).then(function(){ return obj; });
};
...
return centralRequest('', { method:'POST', body:{ action:'bulkUpsert', table(storeName), records } }).then(function(){ return arr; });
};

dbClear = function(storeName){
if(!CENTRAL_TOKEN || !centralTokenValid(CENTRAL_TOKEN)) return _localDbClear(storeName);
return centralRequest('', { method:'POST', body:{ action:'clear', table(storeName) } }).then(function(){ return true; });
};
...
centralRequest('', { method:'POST', body:{ username, password } }).then(function(result){
...
centralRequest('', {method:'POST', body:{action:'delete', table:'colaboradores', record:{rut}}}).then(reloadStateFromDB).then(function(){rerenderAll();showToast('Colaborador eliminado.','success');}).catch(function(err){console.error(err);showToast('No fue posible eliminar el colaborador.','error');});
};
deleteSucursal = function(codigo){
if(!confirm('¿Eliminar esta sucursal?')) return;
centralRequest('', {method:'POST', body:{action:'delete', table:'sucursales', record:{codigo}}}).then(reloadStateFromDB).then(function(){rerenderAll();showToast('Sucursal eliminada.','success');}).catch(function(err){console.error(err);showToast('No fue posible eliminar la sucursal.','error');});
};
eliminarTraslado = function(id){
if(!confirm('¿Eliminar este traslado?')) return;
centralRequest('', {method:'POST', body:{action:'delete', table:'traslados', record:{id}}}).then(reloadStateFromDB).then(function(){return sincronizarTrasladosConColaboradores();}).then(function(){rerenderAll();showToast('Traslado eliminado.','success');}).catch(function(err){console.error(err);showToast('No fue posible eliminar el traslado.','error');});
};
